...
class Lexer:
  ...
  def make_tokens(self):
    ...
    while self.current_char != None:
      if self.current_char in ' \t':
        ...
        # Support integer division token and '//' single-line comments
  ...
  def make_string(self):
    # Support string interpolation with ${expr} syntax
    parts = []  # List of (type, value) where type is 'str' or 'expr'
    pos_start = self.pos.copy()
    ...
    while self.current_char != None and (self.current_char != '"' or escape_character):
      if escape_character:
        current_part += escape_characters.get(self.current_char, self.current_char)
        escape_character = False
        self.advance()
      elif self.current_char == '\\':
        ...
        expr_start = self.pos.copy()
        expr_tokens = []
        depth = 1
        while depth > 0 and self.current_char != None:
          if self.current_char == '{':
            depth += 1
          elif self.current_char == '}':
            depth -= 1
            if depth == 0:
              break
          # Collect character for later parsing
          expr_tokens.append(self.current_char)
          self.advance()
        if depth > 0:
          return None, ExpectedCharError(expr_start, self.pos, "Unclosed ${...}")
        # Store expression as string to parse later
        parts.append(('expr', ''.join(expr_tokens)))
        self.advance()  # skip }
      else:
    ...
    self.advance()
    return Token(TT_STRING, parts, pos_start, self.pos)
  ...
  def make_identifier(self):
    ...
    if upper_id in KEYWORDS:
      return Token(TT_KEYWORD, upper_id, pos_start, self.pos)
  ...
  def skip_comment(self):
    # Skip until end of line or EOF for both '#' and '//' styles
    self.advance()
...
class Parser:
  ...
  def atom(self):
    ...
    elif tok.matches(TT_KEYWORD, 'FUN') or tok.matches(TT_KEYWORD, 'FN'):
  ...
  def if_expr_b_or_c(self):
    ...
    if self.current_tok.matches(TT_KEYWORD, 'ELIF') or self.current_tok.matches(TT_KEYWORD, 'ELSEIF'):
  ...
  def func_def(self):
    ...
    if not (self.current_tok.matches(TT_KEYWORD, 'FUN') or self.current_tok.matches(TT_KEYWORD, 'FN')):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'FUN' or 'FN'"
      ))
...
class BuiltInFunction(BaseFunction):
  ...
  def execute_min(self, exec_ctx):
    ...
    return RTResult().success(Number(min(a.value, b.value)))
  execute_min.arg_names = ["a", "b"]
...
class Interpreter:
  ...
  def visit_StringNode(self, node, context):
    # Handle string interpolation if token value is a list of parts
    if isinstance(node.tok.value, list):
      # String interpolation: parts is list of ('str', str) or ('expr', expr_str)
      res = RTResult()
      result_str = ''
      for part_type, part_value in node.tok.value:
        if part_type == 'str':
          result_str += part_value
        elif part_type == 'expr':
          # Parse and evaluate the expression
          lexer = Lexer(node.tok.pos_start.fn, part_value)
          ...
          expr_value = res.register(self.visit(ast.node, context))
          if res.should_return(): return res
          result_str += str(expr_value)
      return res.success(String(result_str).set_context(context).set_pos(node.pos_start, node.pos_end))
    else:
      # Regular string
      return RTResult().success(
